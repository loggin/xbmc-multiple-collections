...
CVideoInfoScanner::~CVideoInfoScanner()
{
                                                                ...
                                                                CGUIDialogProgress* pDlgProgress)
  {
    ...
    {
      // Always call AddCollection when the movie has a set, so that MSIF local images
      // gathered by UpdateSetInTag are stored even when there is no set NFO file
      // (UpdateSetInTag returns false in that case).
