...
void CVideoDatabase::UpdateTables(int iVersion)
{
  ...
  // The following steps (151-154) are upstream Kodi fixes pulled in by the 2026-08-10 merge of
  // xbmc/xbmc master. They were originally numbered 145-148 upstream, but this fork had already
  // claimed those numbers for the collections work above, so they are renumbered to run after it.
  // This also guarantees any DB that already passed through this fork's own v145-150 (and is
  // therefore already >= 150) still picks these up, since it was never able to run real upstream
  // Kodi's v145-148 under their original numbers.

  if (iVersion < 151)
  {
    // Upstream v145: normalise a legacy audio codec name.
    m_pDS->exec("UPDATE streamdetails set strAudioCodec = 'dts' where strAudioCodec = 'dca'");
  }

  if (iVersion < 152)
  {
    // Upstream v146: recompute episode duration from streamdetails where missing, and drop
    // scraper-created dummy streamdetails rows with no video dimensions.
    constexpr int LOCAL_VIDEODB_ID_EPISODE_RUNTIME = 9;
  ...
  if (iVersion < 153)
  {
    // Upstream v147: sanitise rar:// URL-encoded paths, and normalise DOS-style parent paths
    // that use forward slashes, for both movies and episodes.
  ...
  if (iVersion < 154)
  {
    // Upstream v148: add per-stream source/version columns.
